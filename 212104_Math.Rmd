...
2. 주어진 order와 size를 따르는 그래프에 대한 모든 후보군에 동일 확률 부여. $|V|=N_v$, $|E| = N_e$를 만족하는 모든 그래프 $G=(V,E)$의 collection $\mathcal G_{N_v, N_e}$을 정의하고, 각각의 $G \in \mathcal G_{N_v, N_e}$에 확률 $P(G) = {N \select N_e}^{-1}$을 부여함. 이때 $N= N_v \select 2$는 서로 다른 vertex 2개를 묶은 쌍의 총 숫자.
