...
Approximate κ(θ) using Monte Carlo samples generated from a
distribution f(x|θ
(0)
), where θ
(0)
...
$$
I(\theta) - I(\theta^{(0)}) = \left (\theta - \theta^{(0)} \right) ' g(x_{obs}) - \log \left [ \frac{1}{m} \sum_{i=1}^m \exp \left \{ \left (\theta - \theta^{(0)}\right )'\right g(x_i) \}\right ]
$$
