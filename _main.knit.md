...
1. 주된 관심은 네트워크의 vertex (변호사 케이스라면 변호사 실무) 에 있으며 네트워크 구조 레벨의 속성은 좀 더 역할이 흐릿한 편
...
<img src="_main_files/figure-html/unnamed-chunk-2-1.png" alt="Karate Network" width="672" />
<p class="caption">(\#fig:unnamed-chunk-2)Karate Network</p>
</div>
...
<p class="caption">(\#fig:unnamed-chunk-5)The degree distribution for protein interactions in Yeast</p>
</div>
...
<div class="figure" style="text-align: center">
<img src="_main_files/figure-html/unnamed-chunk-6-1.png" alt="The degree distribution for protein interactions in Yeast" width="672" />
<p class="caption">(\#fig:unnamed-chunk-6)The degree distribution for protein interactions in Yeast</p>
</div>
...
vertex 가 다른 다수의 vertex 와 가깝다면 이를 central 이라고 판정. 일반적으로 사용되는 기준값은 $C_{CL} =\frac{1}{\sum\limits_{u \in V} dist(u,v)}$. 이때 denominator $dist(v, u)$ 는 $u, v \ in V$ 인 vertex $u, v$ 사이의 geodesic distance. 각각 다른 그래프에서 산출된 centrality measure 를 비교하기 위해 normalize 하는 상황 있음. 이때는 factor $N_v - 1$ 으로 곱해서 $[0,1]$ 로 normalize.

- **Betweenness centrality**

어떤 vertex 가 다른 vertex 쌍 사이에 위치하고 있는지를 확인. 이건 vertex 가 네트워크 그래프의 path 에 비추어서 어디에 위치하고 있는지가 중요하다는 관점에 기반. 현실세계에 비추어도 이러한 path 가 인간관계라고 생각한다면 path 가 다수 지나가는 vertex, 즉 인싸는 중요한 사람일 것. 일반적으로 사용되는 값은 $C_B (v) = \sum\limits_{s \not = t \not = v \in V} \frac{\sigma(s,t | v)}{\sigma(s, t)}$. 이때 numerator 는 $v$ 를 통과하면서 $s,t$ 사이가 최단거리인 path 의 총 숫자, denominator 는 이런 조건 없이 $s,t$ 사이가 최단거리인 path 의 총 숫자. 이를 unit interval 로 scale 할 때는 $\frac{(N_v - 1) (N_v - 2)}{2}$ 로 나누면 됨.

- **eigenvector centrality**

‘status’, ‘prestige’, ‘rank’ 등에 기반한 사고방식. vertex 의 이웃이 central 하다면, 본인 vertex 자체도 central 하리라는 관점. 이는 central 의 정의만 생각해보더라도 꽤 합리적인 추론임. 이는 적절하게 정의된 방정식의 linear system 의 ev solution 으로 표현가능함. 이러한 ev centrality 관점에 쓰이는 값은 꽤 많은데 가장 대표적인건 $C_{E_i} (v) = \alpha \sum\limits_{\{u,v\}\in E} C_{E_i}(u)$. 이때 vector $C_{E_i} = \left ( C_{E_i}(1), \cdots, C_{E_i}(N_v) \right)'$ 는 ev problem $A_{C_{E_i}}$ 의 solution. 이때 $A$ 는 네트워크 그래프 $G$ 의 adjacency 매트릭스.
...
<div class="figure" style="text-align: center">
<img src="_main_files/figure-html/unnamed-chunk-9-1.png" alt="Target plots showing various vertex centralities for the karate club network" width="672" />
<p class="caption">(\#fig:unnamed-chunk-9)Target plots showing various vertex centralities for the karate club network</p>
</div>
...
<div class="figure" style="text-align: center">
<img src="_main_files/figure-html/unnamed-chunk-10-1.png" alt="AIDS blog network with vertex area proportional to hubs and authority centrality measures" width="672" />
<p class="caption">(\#fig:unnamed-chunk-10)AIDS blog network with vertex area proportional to hubs and authority centrality measures</p>
</div>
...
<div class="figure" style="text-align: center">
<img src="_main_files/figure-html/unnamed-chunk-13-1.png" alt="Visual representation of the k-core decomposition of the karate network" width="672" />
<p class="caption">(\#fig:unnamed-chunk-13)Visual representation of the k-core decomposition of the karate network</p>
</div>
...
vertex 2개를 뽑아 만든 쌍 (pair) 를 **dyad** 라고 부름. directed 그래프에서 dyad 는 3개의 상태를 가질 수 있다. null (no directed edges), asymmetric (one directed edge), mutual (two directed edges). 대부분의 dyad 는 보통 null 이며, non-null 중에서도 대부분은 aymmetric 이다. 후자의 경우는 블로그에서 서로이웃이 아니라 한쪽만 팔로잉하는 경우겠지.
...
그래프의 **Density** 는 potential, 즉 잠재적으로 발생할 edge 대비 실제로 발생한 edge 들의 빈도. 예를 들어 (undirected) 그래프 $G$ 가 self-loop 가 없고 multiple edge 도 없다고 할 때, 서브그래프 $H=(V_H, E_H)$ 의 density 는 $den(H) = \frac{|E_H|}{\frac{|V_H|(|V_H|-1)}{2}}$. 해당 값은 $[0,1]$ 에 존재하며 $H$ 가 clique 가 되기까지의 역치에 얼마나 가까운지에 대한 측도 (measure) 를 제공함. directed 그래프 라면 denominator 는 $|V_H|(|V_H|-1)$ 로 대체.
...
```r
ego.instr = induced.subgraph(karate, neighborhood(karate, 1,
    1)[[1]])
ego.admin = induced.subgraph(karate, neighborhood(karate, 1,
    34)[[1]])
...
<div class="figure" style="text-align: center">
<img src="_main_files/figure-html/unnamed-chunk-25-1.png" alt="Partitioning of the Karate network obtained from hierarchical clustering" width="672" />
<p class="caption">(\#fig:unnamed-chunk-25)Partitioning of the Karate network obtained from hierarchical clustering</p>
</div>
...
<div class="figure" style="text-align: center">
<img src="_main_files/figure-html/unnamed-chunk-27-1.png" alt="The corresponding dendrogram for this partitioning" width="672" />
<p class="caption">(\#fig:unnamed-chunk-27)The corresponding dendrogram for this partitioning</p>
</div>
...
<div class="figure" style="text-align: center">
<img src="_main_files/figure-html/unnamed-chunk-28-1.png" alt="Eigenvalues of Graph Laplacian" width="672" />
<p class="caption">(\#fig:unnamed-chunk-28)Eigenvalues of Graph Laplacian</p>
</div>
...
<div class="figure" style="text-align: center">
<img src="_main_files/figure-html/unnamed-chunk-29-1.png" alt="Fiedler vector and its corresponding partition" width="672" />
<p class="caption">(\#fig:unnamed-chunk-29)Fiedler vector and its corresponding partition</p>
</div>
...
<img src="_main_files/figure-html/unnamed-chunk-40-2.png" width="672" style="display: block; margin: auto;" />
...
<img src="_main_files/figure-html/unnamed-chunk-41-1.png" width="672" style="display: block; margin: auto;" />
...
<img src="_main_files/figure-html/unnamed-chunk-42-1.png" width="672" style="display: block; margin: auto;" />
...
##    1.00    1.00    1.00    1.98    2.00   20.00
...
<img src="_main_files/figure-html/unnamed-chunk-44-1.png" width="672" style="display: block; margin: auto;" />
...
summary(cl.rg)
##    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
##  0.2150  0.2301  0.2340  0.2341  0.2377  0.2545
summary(apl.rg)
##    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
##   1.812   1.828   1.833   1.833   1.839   1.860
...
<div class="figure" style="text-align: center">
<img src="images/knit-logo.png" alt="Basic Markov Network Statistics" width="32.8%" />
<p class="caption">(\#fig:knitr-logo4)Basic Markov Network Statistics</p>
</div>
...
<div class="figure" style="text-align: center">
<img src="images/knit-logo.png" alt="Degree and Shared Partnership Distribution" width="32.8%" />
<p class="caption">(\#fig:knitr-logo2)Degree and Shared Partnership Distribution</p>
</div>
...
<div class="figure" style="text-align: center">
<img src="images/knit-logo.png" alt="Model Degeneracy for Basic Markov Statistics, Figure: Visualization of the degeneracy (black) and non-degeneracy (white) region of an ERGM with edge counts and K2-star." width="32.8%" /><img src="images/knit-logo.png" alt="Model Degeneracy for Basic Markov Statistics, Figure: Visualization of the degeneracy (black) and non-degeneracy (white) region of an ERGM with edge counts and K2-star." width="32.8%" /><img src="images/knit-logo.png" alt="Model Degeneracy for Basic Markov Statistics, Figure: Visualization of the degeneracy (black) and non-degeneracy (white) region of an ERGM with edge counts and K2-star." width="32.8%" />
<p class="caption">(\#fig:knitr-logo3)Model Degeneracy for Basic Markov Statistics, Figure: Visualization of the degeneracy (black) and non-degeneracy (white) region of an ERGM with edge counts and K2-star.</p>
</div>
...
<p class="caption">(\#fig:ERGMs)Goodness-of-fit Plots, for ERGMs - for the high school student friendship network</p>
</div>
...
1. estimate 되는 Bayes risk[^Bayes risk = Expecation of loss function. 이때 loss function 으로는 Kullback-Leibler loss 를 사용한다.] 를 최소화하는 actor 들의 position 탐색
2. latent position 의 posterior 추출값 (draw) 를 Procrustes Transform 하고 동일한 Transformation Matrix 를 사용하여, cluster mean 와 Cov 를 transform
3. Estimate 된 Bayes risk 를 최소화하는 cluster membership 의 확률을 탐색

이제 이하의 과정을 거쳐서 cluster 의 갯수를 정한다. Bayesian estimation - 이하의 equation, 즉 higehest posterior probability 를 가지는 model 을 선택.
...
<!--chapter:end:990101_Concepts.Rmd-->
...
<!--chapter:end:99010101_Autologistic.Rmd-->
...
<!--chapter:end:99010102_OrderedLogit.Rmd-->
